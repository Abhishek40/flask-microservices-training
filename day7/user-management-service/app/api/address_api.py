...
class AddressApi(Resource):
	...
	def put(self, id, user_id):
		...
		address_db.update_address_details(conn, id, user_id, Address.from_json(request.json))
		address = address_db.get_address_details(conn, id, user_id)
	...
	def delete(self, id, user_id):
		...
		commit_and_close_db_connection(conn)
		return {'message': f'Address [{address["user_id"]}] deleted from the database'}

restful_api.add_resource(AddressApi, '/api/user/<int:user_id>/address/,int:id>')